...
DIのコードは基本的にこのように依存をコンストラクタで渡してオブジェクトが相互依存するオブジェクトグラフを生成します。オブジェクトはル他から所有されているか、他を所有しているか、あるいは双方のいずれかです。
...
次に今`Printer`で固定している挨拶のメッセージも多国語に対応するようため注入するように変更します。
