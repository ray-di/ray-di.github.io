...
プロジェクトの規模が大きくなると、このようなファクトリーコードを使った手動のDIは、深いネストの依存解決、シングルトンなどインスタンス管理、再利用性、メンテナンス性などの問題が現実化してきます。その依存解決の問題を解決するのがRay.Diです。
...
DIパターンとRay.Diの基本を説明しました。

依存はユーザーコードが内部から取得(pull)するのではなく、外部からの注入(inject)が再起的に行われ、オブジェクトグラフが生成されます。

コンパイルタイムでオブジェクトの構成や依存の束縛は完了し、ランタイムではインターフェイスに依存したコードが実行されます。 DIパターンに従う事で、自然にSRP原則[^srp]やDIP原則[^dip]を守る事ができます。

コードから依存を確保する責務が取り除かれ、インターフェイスに依存したコードになりました。コードは安定しながら柔軟で、拡張に対しては開いていても修正に対しては閉じています。[^ocp] 
