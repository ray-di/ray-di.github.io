...
Factoryはオブジェクト作成に複雑なロジックが含まれる場合にも価値があります。集約化により、多段階の初期化、検証、条件付き構築をロジックを再利用可能でテスト可能にします。Factoryはこのロジックを再利用可能かつテスト可能にします。

## Factoryを避けるべき場合

シンプルなケースではFactoryを避けてください。オブジェクトがDI依存関係だけを必要とする場合、コンストラクタインジェクションで十分です。実行時パラメータだけを必要とする場合、シンプルなコンストラクタで十分です。1行のオブジェクト構築のためのFactoryを作成すると、不必要な抽象化が追加されます。

## よくある間違い：静的Factoryメソッド

頻繁に見られるアンチパターンは、DI管理されたFactoryの代わりに静的Factoryメソッドを使用することです：
